...
from pytorchtools import EarlyStopping, DualEarlyStopping
import timeit
import time
...
def test_precess(model, model_drug, pbar, drug_load, LOSS):
    model.eval()
    model_drug.eval()
    test_losses = []
    Y, P, S = [], [], []
    ...
    with torch.no_grad():
        for i, data in pbar:
            ...
            # 使用含标签的forward计算
            predicted_scores, loss= model(
                compounds, num_atoms, proteins, labels)
...
def test_model(model, model_drug, dataset_load, drug_load, save_path, DATASET, LOSS, dataset="Train", lable="best", save=True):
    test_pbar = tqdm(
        enumerate(
    ...
    T, P, loss_test, Accuracy_test, Precision_test, Recall_test, AUC_test, PRC_test = \
        test_precess(model, model_drug, test_pbar, drug_load, LOSS)
    if save:
...
if __name__ == "__main__":
    ...
    torch.manual_seed(SEED)
    train_d, test_d = torch.utils.data.random_split(drug_set, [math.ceil(
    ...
    torch.manual_seed(SEED)
    train_p, test_p = torch.utils.data.random_split(protein_set, [math.ceil(
        0.5*len(protein_set)), len(protein_set)-math.ceil(0.5*len(protein_set))])
    ...
    drug_dataset = MoleculeDataset(
        DATASET + '_drug_inductive_setting_' + str(SEED))
    ...
    early_stopping = DualEarlyStopping(savepath=save_path, patience=hp.Patience, verbose=True, delta=0)
    ...
    for epoch in range(1, hp.Epoch + 1):
        ...
        print(print_msg)
        # early_stopping needs both validation AUC and loss to check if it has decresed,
    ...
    # 加载两个最佳模型进行测试
    print("\n加载最佳AUC模型进行测试...")
    model_auc = torch.load(save_path + '/model_best_auc.pth', weights_only=False)
    model_drug_auc = torch.load(save_path + '/model_drug_best_auc.pth', weights_only=False)
    model_auc.to(device)
    model_drug_auc.to(device)

    print("加载最佳损失模型进行测试...")
    model_loss = torch.load(save_path + '/model_best_loss.pth', weights_only=False)
    model_drug_loss = torch.load(save_path + '/model_drug_best_loss.pth', weights_only=False)
    ...
    model_auc.eval()
    model_drug_auc.eval()
    trainset_test_auc_results, _, _, _, _, _ = test_model(
        model_auc, model_drug_auc, train_dataset_load, train_drug_load, save_path, DATASET, model_auc.compute_loss, dataset="Train", lable="best_auc")
    validset_test_auc_results, _, _, _, _, _ = test_model(
        model_auc, model_drug_auc, valid_dataset_load, valid_drug_load, save_path, DATASET, model_auc.compute_loss, dataset="Valid", lable="best_auc")
    testset_test_auc_results, Accuracy_test_auc, Precision_test_auc, Recall_test_auc, AUC_test_auc, PRC_test_auc = \
        test_model(model_auc, model_drug_auc, test_dataset_load, test_drug_load, save_path,
                   DATASET, model_auc.compute_loss, dataset="Test", lable="best_auc")

    # 测试最佳损失模型
    print("\n测试最佳损失模型:")
    model_loss.eval()
    model_drug_loss.eval()
    trainset_test_loss_results, _, _, _, _, _ = test_model(
        model_loss, model_drug_loss, train_dataset_load, train_drug_load, save_path, DATASET, model_loss.compute_loss, dataset="Train", lable="best_loss")
    validset_test_loss_results, _, _, _, _, _ = test_model(
        model_loss, model_drug_loss, valid_dataset_load, valid_drug_load, save_path, DATASET, model_loss.compute_loss, dataset="Valid", lable="best_loss")
    testset_test_loss_results, Accuracy_test_loss, Precision_test_loss, Recall_test_loss, AUC_test_loss, PRC_test_loss = \
        test_model(model_loss, model_drug_loss, test_dataset_load, test_drug_load, save_path,
                   DATASET, model_loss.compute_loss, dataset="Test", lable="best_loss")
    ...
    print("\n模型性能对比:")
    print("=" * 80)
    print(f"{'指标':<15} {'最佳AUC模型':<20} {'最佳损失模型':<20} {'差异':<15}")
    print("-" * 80)
    
    # 计算并显示各个指标的对比
    metrics = {
        'Accuracy': (Accuracy_test_auc, Accuracy_test_loss),
        'Precision': (Precision_test_auc, Precision_test_loss),
        'Recall': (Recall_test_auc, Recall_test_loss),
        'AUC': (AUC_test_auc, AUC_test_loss),
        'AUPR': (PRC_test_auc, PRC_test_loss)
    }
    
    for metric, (auc_value, loss_value) in metrics.items():
        diff = auc_value - loss_value
        diff_str = f"{diff:+.4f}" if abs(diff) > 0.0001 else "≈0"
        print(f"{metric:<15} {auc_value:<20.4f} {loss_value:<20.4f} {diff_str:<15}")
    ...
    # 同时保存到文件
    with open(save_path + "/model_comparison.txt", 'w') as f:
        f.write("模型性能对比:\n")
        f.write("=" * 80 + "\n")
        f.write(f"{'指标':<15} {'最佳AUC模型':<20} {'最佳损失模型':<20} {'差异':<15}\n")
        f.write("-" * 80 + "\n")
        
        for metric, (auc_value, loss_value) in metrics.items():
            diff = auc_value - loss_value
            diff_str = f"{diff:+.4f}" if abs(diff) > 0.0001 else "≈0"
            f.write(f"{metric:<15} {auc_value:<20.4f} {loss_value:<20.4f} {diff_str:<15}\n")
        
        f.write("=" * 80 + "\n")
        f.write(f"最佳AUC模型在epoch: {early_stopping.best_auc_epoch}\n")
        f.write(f"最佳损失模型在epoch: {early_stopping.best_loss_epoch}\n")
        f.write("=" * 80 + "\n")